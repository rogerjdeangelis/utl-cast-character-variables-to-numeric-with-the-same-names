Cast character variables to numeric with the same names
    ...
    Four Solutions ( two sets: one simpler with warnings and one without warnings)
          ...
          1. Change just character variables that begin with 'R' to numeric (with and without warnigs)
    ...
    This solution use
       ...
       Bart Jablonski  (has added a dataset funtionality, missing in the macro language to do_over and array)
